...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            labelBox = tools.compute_box_corners(label.box.center_x, label.box.center_y, label.box.width, label.box.length, label.box.heading)
            ## step 2 : loop over all detected objects

            for detection in detections:
                ## step 3 : extract the four corners of the current detection
                _id, x, y,z, _h, w, l, yaw = np.array((detection))
                detectBox = tools.compute_box_corners(x, y, w, l, yaw)
                ...
                x1_intersection = max(x1_label, x1_detection)
                y1_intersection = max(y1_label, y1_detection)
                x2_intersection = min(x2_label, x2_detection)
                y2_intersection = min(y2_label, y2_detection)

                # Calculate the area of intersection
                intersection_area = max(0, x2_intersection - x1_intersection + 1) * max(0, y2_intersection - y1_intersection + 1)

                # Calculate the area of the label and detection bounding boxes
                label_area = (x2_label - x1_label + 1) * (y2_label - y1_label + 1)
                detection_area = (x2_detection - x1_detection + 1) * (y2_detection - y1_detection + 1)

                # Calculate the area of union
                union_area = label_area + detection_area - intersection_area
...
def compute_performance_stats(det_performance_all):
    ...
    for pos_neg in pos_negs:
        positives = positives + pos_neg[0]
        true_positives = true_positives + pos_neg[1]
        false_negatives = false_negatives + pos_neg[2] 
        false_positives = false_positives + pos_neg[3]
