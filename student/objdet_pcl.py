...
def bev_from_pcl(lidar_pcl, configs):
    ...
    colStep = (configs.lim_x[1] - configs.lim_x[0])/configs.bev_height  
    rowStep = (configs.lim_y[1] - configs.lim_y[0])/configs.bev_width  
    
    ## step 2 : create a copy of the lidar pcl and transform all metrix x-coordinates into bev-image coordinates    
    lidar_pcl_cpy = np.copy(lidar_pcl)
    lidar_pcl_cpy[:, 0] = np.int_((lidar_pcl_cpy[:, 0] - configs.lim_x[0])/colStep)

    # step 3 : perform the same operation as in step 2 for the y-coordinates but make sure that no negative bev-coordinates occur
    lidar_pcl_cpy[:, 1] = np.int_(np.floor(lidar_pcl_cpy[:, 1] / rowStep) + (configs.bev_width + 1) / 2)
